...
const Subscription = () => {
    ...
    const subscriptionData = {
        ...
        subcontractor: [
            {
                id: 1, name: 'Basic Plan', price: '$39', duration: 'Monthly', features: ['Basic features', 'Up to 10 crew members', 'Up to 1 project', 'Up to 3 team members'],
                isPopuler: false
            },
            {
                id: 2, name: 'Standard Plan', price: '$69', duration: 'Monthly', features: ['Standard features', 'Up to 20 crew members', 'Up to 3 projects', 'Up to 5 team members'],
                isPopuler: true
            },
            {
                id: 3, name: 'Premium Plan', price: '$89', duration: 'Monthly', features: ['Premium features', 'Unlimited crew members', 'Unlimited projects', 'Unlimited team members'],
                isPopuler: false
            },
        ]
    };
    ...
    return (
        <section className='container xl:px-0 px-5 py-8 md:py-12'>
            <div className='mb-5'>
                <h2 className='xl:text-5xl text-3xl font-bold mb-3 text-gray-800'>Choose the Right Plan for Your Business</h2>
                <p className='text-lg text-gray-700'>
                    Flexible membership options for contractors and subcontractors — whether you're just starting or scaling your crew.
                </p>
            </div>

            {/* Tabs */}
            <div className='relative flex my-10 justify-center items-center'>
                <img className='w-full max-w-82 ' src="/Images/Home/subscription-tab-bg.png" alt="" />
                <div className='flex items-center gap-2 justify-center w-full h-full absolute top-0 left-0 mb-5'>
                    <button
                        className={`px-7 cursor-pointer rounded-lg py-3 font-semibold ${activeTab === 'contractor' ? 'bg-[#bac3de] text-[#333]' : 'bg-gray-200 text-gray-700'}`}
                        onClick={() => handleTabClick('contractor')}
                    >
                        Contractor
                    </button>
                    <button
                        className={`px-7 cursor-pointer rounded-lg py-3 font-semibold ${activeTab === 'subcontractor' ? 'bg-[#bac3de] text-[#333]' : 'bg-gray-200 text-gray-700'}`}
                        onClick={() => handleTabClick('subcontractor')}
                    >
                        Subcontractor
                    </button>
                </div>
