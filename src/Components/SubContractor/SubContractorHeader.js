    ...
    return (
        <header className=' flex flex-wrap gap-5 items-center justify-center md:justify-between py-5'>
            ...
            <div className='flex items-center gap-5'>
                ...
                <Link href="/sub-contractor/message" className='cursor-pointer px-2 relative text-white py-2 border bg-[#203f9a] rounded-md'>
                    {icons.message}
                    <span className='absolute top-2 right-2 bg-red-600 w-2 h-2 rounded-full'></span>
                </Link>
                <Link href="/sub-contractor/notifications" className='cursor-pointer px-2 relative text-white py-2 border bg-[#203f9a] rounded-md'>
                    {icons.notification}
                    <span className='absolute top-2 right-2 bg-red-600 w-2 h-2 rounded-full'></span>
                </Link>
                <div className='relative'>
                    <img onClick={handleSowUser} className='w-12 cursor-pointer' src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQOtu74pEiq7ofeQeTsco0migV16zZoBwSlGg&s" alt="" />
                    {
                        ...
                        <div className='absolute border border-gray-300 z-10 top-14 min-w-[300px] right-0 px-2 py-5 bg-gray-100 rounded-lg'>
                            ...
                            <h3 className='text-center text-[#203f9a] font-semibold'>sub-contractor</h3>
                            <div className='bg-white mt-5 font-semibold py-2 px-5 rounded-lg flex flex-col gap-2 '>
                                <Link onClick={handleSowUser} href={'/sub-contractor/profile'} className='flex items-center gap-2 hover:text-[#203f9a] py-2 cursor-pointer border-b border-gray-300'>
                                ...
                                {/* setting  */}
                                <Link onClick={handleSowUser} href={'/sub-contractor/setting'} className='flex items-center gap-2 hover:text-[#203f9a] py-2 cursor-pointer border-b border-gray-300'>
                                    <span>{icons.setting}</span>
                                    <span>Setting</span>
                                </Link>
                                <Link onClick={handleSowUser} href={'/sub-contractor/terms-privacy'} className='flex items-center gap-2 hover:text-[#203f9a] py-2 cursor-pointer border-b border-gray-300'>
