    ...
    <div className="w-full p-4 lg:p-0 mb-2">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <button
          ...
          className="text-sm text-[#1f3d95] cursor-pointer font-medium"
        >
          Mark All as Read
        </button>
      </div>
      ...
      <div className="space-y-2">
        {currentNotifications.map((notification) => (
          <div
            key={notification.id}
            className={`flex items-start space-x-4 p-3 cursor-pointer rounded-lg ${!notification.read ? 'bg-[#1f3d951e]' : 'bg-white'}  border-2 border-[#1f3d951e]`}
          >
        ...
        <span className="flex items-center space-x-2">
          {Array.from({ length: totalPages }).map((_, index) => (
            <button
              ...
              className={`px-4 py-2 rounded-md ${currentPage === index + 1 ? 'bg-[#1f3d95] text-white' : 'bg-white text-[#1f3d95]'}`}
            >
              {index + 1}
            </button>
          ))}
